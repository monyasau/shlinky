...
import { linkType, userType } from "@/types/type";
...
export default function Link({ link, removeLink, index }: LinkProps) {
  ...
  return (
    <div key={link.id} className="bg-[#fafafa] rounded-xl p-5">
      <div className="flex mb-4 justify-between">
        ...
        <a href={shareUrl} className="cursor-pointer">
          <code>{shareUrl}</code>
        </a>
        <div className="inline-flex gap-4">
          <button className="text-[#737373] " onClick={handleShare}>
            Share
          </button>
          <button
      ...
      <div className="flex justify-end pt-2">
        <button
          onClick={updateLink}
          className="customShadow py-2 px-4 text-white text-sm bg-[#633CFF] rounded-lg hover:bg-[#BEADFF] border-none"
        >
          Saves
        </button>
      </div>
