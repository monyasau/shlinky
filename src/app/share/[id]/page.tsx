  ...
  useEffect(() => {
    if (id) {
      const fetchLink = async () => {
        const docRef = doc(db, "links", id as string);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          setLink(docSnap.data() as linkType);
        } else {
...
const lowerCaseUrl =link.url.toLowerCase();
  ...
  title={link.title}></iframe>
      ...
      <p className="text-lg">
        URL: <a href={`https://${lowerCaseUrl}`} target="_blank" rel="noopener noreferrer" className=" lowercase">https://{link.url}</a>
      </p>
      <a href={`https://${lowerCaseUrl}`} target="_blank" rel="noopener noreferrer" className="w-[90%] text-white">
